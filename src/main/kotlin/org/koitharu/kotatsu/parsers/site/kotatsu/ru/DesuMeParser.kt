...
internal class DesuMeParser(context: MangaLoaderContext) :
	...
	override suspend fun getPages(chapter: MangaChapter): List<MangaPage> {
		...
		val scriptData = document.selectFirst("script:containsData(window.MangaReader)")?.data()
			?.substringAfter("window.MangaReader =")
			?.substringBeforeLast(';')
			?.trim()
			?.takeIf { it.isNotEmpty() }
			?: error("Cannot find Desu reader state")
		val readerState = JSONObject(scriptData)
		val chapterId = readerState.getJSONObject("chapter").getLong("id")
		val apiBaseUrl = readerState.getString("apiBaseUrl")
		val apiUrl = "$apiBaseUrl/chapters/$chapterId".toAbsoluteUrl(domain)
		val images = webClient.httpGet(apiUrl).parseJson()
			.getJSONObject("chapter")
			.getJSONArray("pages")

		return (0 until images.length()).map { index ->
			val imageUrl = images.getJSONObject(index).getString("url")
			MangaPage(
