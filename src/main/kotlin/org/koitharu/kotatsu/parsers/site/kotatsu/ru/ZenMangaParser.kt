...
internal class ZenMangaParser(context: MangaLoaderContext) :
	...
	private fun checkAuth(): Boolean {
		...
		return context.cookieJar.getCookies(domain).any { it.name == authCookieName }
	}
	...
	override suspend fun getUsername(): String {
		...
		val data = fetchAstroData(libraryUrl)
			?: throw ParseException("Cannot obtain Astro state for the current user", libraryUrl)

		val session = data["session"] as? Map<*, *>
			?: throw ParseException("Cannot find the 'session' key", libraryUrl)

		val currentUser = session["currentUser"] as? Map<*, *>
			?: throw ParseException("Cannot find the 'currentUser' key", libraryUrl)

		return currentUser["username"] as? String
			?: throw ParseException("Cannot find the 'username' key", libraryUrl)
	}
	...
	override suspend fun getDetails(manga: Manga): Manga {
		val data = fetchAstroData(manga.url)
			?: throw ParseException("Cannot obtain Astro state for manga details", manga.publicUrl)
	...
	override suspend fun getPages(chapter: MangaChapter): List<MangaPage> {
		val data = fetchAstroData(chapter.url)
			?: throw ParseException("Cannot obtain Astro state for chapter pages", chapter.url)

		val chapterData = data["reader-current-chapter"] as? Map<*, *>
			?: throw ParseException("Cannot find the 'reader-current-chapter' key", chapter.url)

		val pagesList = chapterData["pages"] as? List<Map<*, *>>
			?: throw ParseException("Cannot find the 'pages' list", chapter.url)
	...
	private suspend fun fetchAstroData(relativeUrl: String): Map<*, *>? {
		...
		val scriptElement = responseHtml.getElementById("it-astro-state")
			?: throw ParseException("Cannot find <script id='it-astro-state'> at $fullUrl", fullUrl)
