package org.koitharu.kotatsu.parsers.site.tachiyomi.en.scansgg
...
@TachiyomiSource("SCANSGG", "ScansGG", "en")
...
// ============================= Chapters ==============================
    ...
    private fun chapterListParseApi(response: Response): List<SChapter> {
        val mangaId = response.request.url.queryParameter("series_id")
            ?: throw Exception("Missing series id")

        val dto = response.parseAs<ResponseDto<List<ChapterDto>>>()

        return dto.data.map {
            it.toSChapter(mangaId, dateFormat)
        }
    }
