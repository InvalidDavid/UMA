package org.koitharu.kotatsu.parsers.site.tachiyomi.en.onisaga
...
@TachiyomiSource("ONISAGA_EN", "OniSaga (English)", "en")
class OniSagaEN : OniSaga(
    lang = "en",
    langKey = "EN",
)

@TachiyomiSource("ONISAGA_FR", "OniSaga (French)", "fr")
class OniSagaFR : OniSaga(
    lang = "fr",
    langKey = "FR",
)

@TachiyomiSource("ONISAGA_JA", "OniSaga (Japanese)", "ja")
class OniSagaJA : OniSaga(
    lang = "ja",
    langKey = "JA",
)
...
abstract class OniSaga(
    ...
    override fun fetchPopularManga(page: Int): Observable<MangasPage> {
        ...
        return client.newCall(
            GET("$baseUrl/browse", headers),
        )
            .asObservable()
            .map { response ->
                response.close()
                fetchMangaLivewirePage(
                    "$baseUrl/browse",
                    page,
                    updates,
                )
    ...
    private fun fetchMangaLivewirePage(
        ...
        val dto = client.newCall(
            POST("$baseUrl/livewire/update", buildLivewireHeaders(url.substringBefore("?")), request.toJsonRequestBody()),
        ...
        Observable.fromCallable {
            ...
            if (query.startsWith("http")) {
                ...
                if (url.pathSegments.firstOrNull() == "read") {
                    val doc = client.newCall(GET(query, headers))
                        .execute()
                        .use { it.asJsoup() }

                    mangaUrl = doc.selectFirst("a[href*=\"/manga/\"]")
                        ?.absUrl("href")
                        ?: throw Exception("Could not find manga link")
                } else if (
                    url.pathSegments.size >= 3 &&
                    url.pathSegments[0] == "manga"
                ) {
                    mangaUrl =
                        "${url.scheme}://${url.host}/manga/${url.pathSegments[1]}"
                }

                val manga = client.newCall(GET(mangaUrl, headers))
                    .execute()
            ...
            fetchMangaLivewirePage(
                ...
                page,
                buildUpdatesFromFilters(filters),
            )
        }
        ...
        client.newCall(mangaDetailsRequest(manga))
            ...
            .map { response ->
                ...
                while (true) {
                    ...
                    val chapterRequest = POST(
                        "$baseUrl/livewire/update",
                        buildLivewireHeaders(baseUrl + manga.url),
                        request.toJsonRequestBody(),
                    )

                    val dto = client.newCall(chapterRequest)
                        .execute()
                        .use { chapterResponse ->
                            if (!chapterResponse.isSuccessful) {
                                null
                            } else {
                                chapterResponse.parseAs<LivewireResponse>()
                            }
                        }
                        ?: break

                    val html = dto.components.firstOrNull()
                        ?.effects
                        ?.html
                        ?: break

                    val newChapters = parseChaptersFromDoc(
                ...
                chapters
                    .distinctBy { it.url }
                    .sortedByDescending {
                        CHAPTER_NUMBER_REGEX.find(it.name)
                            ?.groupValues
                            ?.get(1)
                            ?.toFloatOrNull()
                            ?: 0f
    ...
    override fun fetchImageUrl(page: Page): Observable<String> {
        ...
        return Observable.fromCallable {
            ...
            while (attempt < 3) {
                ...
                val response = pageClient.newCall(
                    GET(
                        apiUrl,
                        apiHeaders(token, chapterUrl),
                    ),
                ).execute()

                val nextToken = response.header("x-reader-token-next")
                if (!nextToken.isNullOrBlank()) {
                    token = nextToken
                    currentReaderToken = nextToken
                }
                ...
                if (!contentType.contains("json", ignoreCase = true)) {
                    ...
                    val refreshedToken =
                        READER_TOKEN_REGEX.find(refresh)
                            ?.groupValues
                            ?.get(1)

                    if (!refreshedToken.isNullOrBlank()) {
                        token = refreshedToken
                        currentReaderToken = refreshedToken
                        continue
                    }

                    throw Exception(
                        "OniSaga reader returned HTML: ${body.take(100)}"
                    )
                }

                val dto = body.parseAs<PageApiResponse>()

                val image = dto.url
                    ?: throw Exception("No image URL returned: ${dto.message}")

                return@fromCallable image
            }

            throw Exception("Failed fetching image URL")
        }
    }
