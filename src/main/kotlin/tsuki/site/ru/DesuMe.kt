...
internal class DesuMe(context: MangaLoaderContext) :
...
internal fun Document.parseDesuTags(): Set<MangaTag> =
    select("#animeFilter input[data-genre-id][data-genre-slug][data-genre-name]").mapTo(linkedSetOf()) { input ->
        MangaTag(
            "${input.attr("data-genre-id")}-${input.attr("data-genre-slug")}",
            input.attr("data-genre-name").toTitleCase(),
            MangaParserSource.DESUME,
        )
    }
