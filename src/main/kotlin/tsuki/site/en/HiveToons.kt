...
internal class HiveToons(context: MangaLoaderContext):
    PagedMangaParser(context, MangaParserSource.HIVETOONS, 20) {
    ...
    private val statusTags = listOf(
        "Ongoing" to "status_ongoing",
        "Completed" to "status_completed",
        "Canceled" to "status_canceled",
        "Dropped" to "status_dropped",
        "Coming Soon" to "status_coming_soon",
    )

    private val typeTags = listOf(
        "Manga" to "type_manga",
        "Manhua" to "type_manhua",
        "Manhwa" to "type_manhwa",
        "Russian" to "type_russian",
        "Spanish" to "type_spanish",
    )
    ...
    override suspend fun getFilterOptions(): MangaListFilterOptions {
        ...
        statusTags.forEach { (name, key) -> tags.add(MangaTag(name, key, source)) }
        typeTags.forEach { (name, key) -> tags.add(MangaTag(name, key, source)) }
    ...
    private fun JSONArray.toMangaList(): List<Manga> {
    ...
    override suspend fun getDetails(manga: Manga): Manga {
        ...
        val finalChaptersArr = if (totalChapters > chaptersArr.length()) {
            val chaptersJson = webClient.httpGet("$apiBaseUrl/api/chapters?postId=${id.toInt()}", getRequestHeaders()).parseJson()
            chaptersJson.optJSONObject("post")?.optJSONArray("chapters") ?: JSONArray()
        } else {
            chaptersArr
        }

        val chapters = parseChaptersFromArray(finalChaptersArr, slug)
