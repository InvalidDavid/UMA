...
internal class ComicLand(context: MangaLoaderContext) : PagedMangaParser(
    ...
    override suspend fun getListPage(page: Int, order: SortOrder, filter: MangaListFilter): List<Manga> {
        ...
        val endpoint =
            if (!filter.query.isNullOrBlank()) {
                ...
                val category = filter.tags.firstOrNull()?.key
                when (category) {
                    "official" -> "$api/comics/official"
                    "ongoing" -> "$api/comics"
                    "popular" -> "$api/comics/popular"
                    else -> "$api/comics"
                }
            }

        val separator = if (endpoint.contains("?")) "&" else "?"
        val response = webClient.httpGet("$endpoint${separator}offset=$offset&limit=20")
    ...
    override suspend fun getDetails(manga: Manga): Manga {
        val response = webClient.httpGet("$api/comic/detail?slug=${manga.url}")
        val root = JSONObject(response.body.string())
        val comic = root.optJSONObject("data")
                ?: return manga
        val chapters = ArrayList<MangaChapter>()
        val chapterArray = comic.optJSONArray("chapters")

        if (chapterArray != null) {
            for (i in 0 until chapterArray.length()) {
                val chapter = chapterArray.getJSONObject(i)

                val index = chapter.optDouble("chapter_index", 0.0)
                chapters.add(
                    MangaChapter(
        ...
        return manga.copy(
            ...
            coverUrl = comic.optString("cover_url", manga.coverUrl),
            authors = parseNames(comic.optJSONArray("authors")),
            tags = emptySet(),
            chapters = chapters
        )
    }

    private fun parseNames(array: org.json.JSONArray?): Set<String> {
        if (array == null)
            return emptySet()
        return buildSet {
            for (i in 0 until array.length()) {
    ...
    override suspend fun getPages(chapter: MangaChapter): List<MangaPage> {
        ...
        val index = chapter.url
                .substringAfter("/chapter/")
        val response = webClient.httpGet("$api/chapter/pages_by_index?slug=$slug&index=$index")
        val root = JSONObject(response.body.string())
        val data = root.optJSONObject("data")
                ?: return emptyList()
