...
internal class Mangataro(context: MangaLoaderContext) :
    ...
    private suspend fun loadChapters(mangaId: String, referer: String): List<MangaChapter> {
        ...
        while (offset < expectedTotal) {
            ...
            if (raw.isEmpty()) {
                throw ParseException("Chapter API returned an empty response", requestUrl)
            }
            val json = JSONObject(raw)
            if (!json.optBoolean("success", false)) {
                throw ParseException("Chapter API rejected the request", requestUrl)
            }
            expectedTotal = json.optInt("total", expectedTotal)
            val chaptersArray = json.optJSONArray("chapters")
                ?: throw ParseException("Chapter API response has no chapters array", requestUrl)
