...
internal class VyManga(context: MangaLoaderContext) :
    ...
    override val configKeyDomain: ConfigKey.Domain = ConfigKey.Domain("mangavyvy.net")
    ...
    private suspend fun fetchAvailableTags(): Set<MangaTag> {
        ...
        doc.select(".check-genre div div:has(.checkbox-genre)").forEach { element ->
            val label = element.selectFirst("label")?.text()?.trim()
            val input = element.selectFirst(".checkbox-genre")
            val key = input?.attr("data-value")?.trim()

            if (!label.isNullOrBlank() && !key.isNullOrBlank()) {
                tags += MangaTag(
                    title = label,
                    key = key,
                    source = source,
