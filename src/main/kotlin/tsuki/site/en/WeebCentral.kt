...
internal class WeebCentral(context: MangaLoaderContext) : AbstractMangaParser(context, MangaParserSource.WEEBCENTRAL),
    ...
    override suspend fun getFilterOptions(): MangaListFilterOptions {
        ...
        val states = EnumSet.of(
            MangaState.ONGOING, MangaState.FINISHED, MangaState.ABANDONED, MangaState.PAUSED,
        )
        ...
        val url = "https://$domain/search/data".toHttpUrl().newBuilder().apply {
            ...
            addQueryParameter(
                ...
                value = when (order) {
                    ...
                    SortOrder.ADDED, SortOrder.ADDED_ASC -> "Recently Added"
            ...
            addQueryParameter(
                name = "order",
                value = when (order) {
                    SortOrder.RELEVANCE, SortOrder.ALPHABETICAL, SortOrder.POPULARITY_ASC, SortOrder.RATING_ASC, SortOrder.ADDED_ASC, SortOrder.UPDATED_ASC -> "Ascending"
                    SortOrder.ALPHABETICAL_DESC, SortOrder.POPULARITY, SortOrder.RATING, SortOrder.ADDED, SortOrder.UPDATED -> "Descending"
                    else -> throw UnsupportedOperationException("unsupported order: $order")
                },
            )
            ...
            with(filter.contentRating) {
                addQueryParameter(
                    ...
                    value = when {
                        isEmpty() -> "Any"
                        ContentRating.SAFE in this && ContentRating.SUGGESTIVE in this -> "Any"
                        ContentRating.SAFE in this -> "False"
                        ContentRating.SUGGESTIVE in this -> "True"
