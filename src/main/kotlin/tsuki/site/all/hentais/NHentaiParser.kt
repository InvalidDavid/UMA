...
internal class NHentaiParser(context: MangaLoaderContext) :
    ...
    override val availableSortOrders: Set<SortOrder> = EnumSet.of(
        SortOrder.UPDATED,      // latest (default)
        SortOrder.POPULARITY,   // popular (all time)
...
11924=kagerou imaizumi
    ...
    private fun JSONObject.toMangaDetails(): Manga {
        ...
        for (tag in tags) {
            ...
            if (parts.size == 2 && parts[0] == "language") {
                val name = parts[1]
                when {
                    name.equals("translated", ignoreCase = true) -> translated = true
                    else -> languageCandidates.add(name)
                }
            }
        }

        langName = languageCandidates.firstOrNull { it.equals("chinese", ignoreCase = true) }
        ...
        val description = buildString {
            langName?.let {
                val displayLang = it.replaceFirstChar { c -> c.uppercase() }
                append(if (translated) "$displayLang (translated)" else displayLang)
                append("\n")
            }
