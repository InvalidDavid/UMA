...
`HENTAINEXUS` is not an addition because the local catalog already contains it. `HIPERDEX` is excluded because the UMA source is marked broken.
...
`ACOMICS`, `BEST_MANGA`, `HENTAILIB`, and `ZENMANGA` remain exposed with `isBroken=true` until repair contracts pass against viable endpoints.
